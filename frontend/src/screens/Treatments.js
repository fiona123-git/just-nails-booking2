...
function Treatments() {
  ...
  const [date, setDate] = useState(); // create treatment state
  ...
  useEffect(() => {
    ...
    if (!token) {
      // if it isnt the right token then go to login
      navigate("/login");
      return;
    }
    axios
      ...
      .catch((e) => {
        // catch the error will result to navigate to login
        navigate("/login");
      });
  }, []);
        ...
        <tbody>
          ...
          {treatment.map((record) => (
            <tr key={record._id}>
              ...
              <td>
                <input type="text" defaultValue={record.therapy} />
              </td>
              <td>
                <input type="text" defaultValue={record.description} />
              </td>
              <td>
                <input type="text" defaultValue={record.price} />
              </td>
              <td>
                <input
                  type="datetime-local"
